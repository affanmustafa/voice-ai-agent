...
Single-call demo for the SF-ENG-0042 Voice AI Pipeline take-home. The project has a Python realtime voice loop, a TypeScript API layer, and a React transcript UI.
...
The assignment allows mocked providers. This implementation uses OpenAI Realtime as the single-provider shortcut instead of a mock STT -> LLM -> TTS chain. The trade-off is less control over internal provider stages, but much more realistic transcription and generated speech while keeping the local architecture small.
...
The transcript intentionally keeps small overlap between the interrupted agent turn and the next user turn. That mirrors real detection/playback latency and matches the assignment example.
...
Example live run numbers from this demo were approximately:

```json
{
  "stt_first_final_ms": 864,
  "llm_first_token_ms": 1532,
  "tts_first_byte_ms": 1621,
  "voice_to_voice_ms": 1621
}
```
...
```json
{
  ...
  "started_at": "2026-06-07T19:17:35.726000+00:00",
